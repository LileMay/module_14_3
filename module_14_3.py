...
product_inline_choices = InlineKeyboardMarkup(
    inline_keyboard=[
...
async def get_buying_list(message):
    for product in range(1, 5):
        await message.answer(f'Название: Product{product} |'
                             f' Описание: описание {product} | Цена: {product * 100}')
        with open(f'images/img{product}.png', 'rb') as photo:
            await message.answer_photo(photo)
    await message.answer("Выберите продукт для покупки:",
                         reply_markup=product_inline_choices)
